...
class pc_base {
	 ...
	 * @param string $classname 类名
	 */
	public static function load_apimodel($classname) {
		return self::_load_api_class($classname,'/api/model/');
	}
	 ...
	 * @param intger $initialize 是否初始化
	 */
	private static function _load_api_class($classname, $path = '', $initialize = 1) {
		...
		if (file_exists(PHPCMS_PATH.$path.DIRECTORY_SEPARATOR.$classname.'.class.php')) {
			include PHPCMS_PATH.$path.DIRECTORY_SEPARATOR.$classname.'.class.php';
			$name = $classname;
			if ($my_path = self::my_path(PHPCMS_PATH.$path.DIRECTORY_SEPARATOR.$classname.'.class.php')) {
				include $my_path;
				$name = 'MY_'.$classname;
			}
			if ($initialize) {
